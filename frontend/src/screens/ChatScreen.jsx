...
import { sanitizeClientPII, detectPIIPresence } from "../anonymizer.js";
      ...
      cleanMd(
        await callClaude({
          ...
          messages: history.slice(-6).map((m) => ({
            role: m.role,
            content: sanitizeClientPII(m.text || m.content || ""),
          })),
        }),
      ) || t.noAnswer;
    let answer = raw,
...
async function handleDocumentUpload(e) {
  ...
  async function runTool(kind) {
    ...
    setCheckedDocs({});
    try {
      const userContext = sanitizeClientPII(getChatContext());
      ...
      >
        ...
        {detectPIIPresence(input) && (
          ...
          >
            ...
            <span>
              {lang === "hi"
                ? "शील्ड सक्रिय: संवेदनशील जानकारी (फ़ोन/आधार/पैन) स्थानीय रूप से सुरक्षित (Masked) कर दी जाएगी।"
                : "Privacy Shield Active: Sensitive details (Phone/Aadhaar/PAN) masked on-device before sending."}
            </span>
          </div>
        )}
      </div>
